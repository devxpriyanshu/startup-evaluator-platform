/* EIL EngSUI Commercialisation Intelligence Tool — static app */
(function () {
  'use strict';
  ...
  const DEMO_STARTUPS = [
    {
      ...
      form: {
        ...
        'p-eval': 'EIL Strategy · Demo', 'p-founder-ask': '55', 'p-notes': 'Strong EPCM alignment; National Hydrogen Mission anchor candidate.',
    ...
    },
    {
      ...
      form: {
        ...
        'p-eval': 'EIL Strategy · Demo', 'p-founder-ask': '22', 'p-notes': 'Pilot with mid-size cement plant signed; paid field trial underway.',
    ...
    },
    {
      ...
      form: {
        ...
        'p-sec': 'solar', 'p-stage': 'seed', 'p-hq': 'Delhi', 'p-eval': 'EIL Strategy · Demo',
    ...
    },
    {
      ...
      // commercially excellent startup that should REJECT on EIL strategic-fit hard gate
      ...
      form: {
        ...
        'p-eval': 'EIL Strategy · Demo', 'p-founder-ask': '',
        'p-notes': 'Series D raised Nov 2025; cumulative funding ~$102M (IvyCap, Chiratae, + 120 investors). Consumer deep-tech — deliberately included to stress-test the EIL-fit hard gate. Expected outcome: high CRS on tech/market/team, REJECT on engagement (EIL fit < 3).',
    ...
    },
    {
      ...
      // Positive EIL-fit validation case: opposite of Miko — lower CRS on IP/business maturity but
      // squarely in the energy-transition thesis (CCUS / Net Zero critical path). Expected: passes
      // EIL gate, likely "Promising" or "Commercially ready" verdict, engagement = engineering or license.
      ...
      form: {
        ...
        'p-eval': 'EIL Strategy · Demo', 'p-founder-ask': '',
        'p-notes': 'First Indian startup to secure Frontier (Stripe-led) CDR offtake. Founders: Shrey &amp; Sparsh Agarwal. Validation case: strong EIL alignment (CCUS / Net Zero), early-stage business maturity, established ERW science (low IP moat — depends on execution + MRV rigour).',
    ...
    },
    {
      ...
      // because Fermentech is ALREADY an EngSUI seed-grant recipient (so EIL has prior
      ...
      form: {
        ...
        'vd-market-ref': '0', 'vd-market-ref-note': 'No disclosed valuation or institutional priced round. Paid-up capital ₹50,910; only non-dilutive grants (DBT BIG, BIRAC, EngSUI) confirmed — market reference not applicable at this stage.',
    ...
    },
    {
      ...
      // energy-PSU reference that stress-tests the EIL-fit / EPC scoring axes.
    ...
    },
    {
      ...
      // Expected EIL engagement: LICENSE (strong IP + energy-storage thesis, limited direct EPC fit).
      name: 'Log9 Materials',
      form: {
    ...
    },
    {
      ...
      // CRITICAL validation case: Engineers India Limited (EIL) is already listed as an
      // investor/supporter by Tracxn alongside HDFC Capital and RTBI — this is a live EIL
      // portfolio company. Platform should correctly output EQUITY engagement, confirming what
      // EIL has already done. Co-founder Prof. Thalappil Pradeep (IISc PhD, Padma Shri, 500+
      ...
      form: {
        ...
        'vd-market-ref': '0', 'vd-market-ref-note': 'No priced equity round disclosed. RTBI (5% cap-table equity) and implied EIL + HDFC Capital involvement appears to be through incubation grants — no post-money valuation available. Revenue Multiple on trailing ARR (₹0.638 Cr × 6× water sector) gives directional ₹3–4 Cr — not a defensible anchor at this scale.',
        ...
        'vd-legal-dd': 'in-progress',
        'vd-legal-notes': 'Negative equity (₹33.1L accumulated deficit) and total liabilities ₹1.6 Cr vs. assets ₹1.3 Cr — balance sheet is technically insolvent; verify no creditor enforcement or winding-up risk. Verify exact legal instrument by which EIL and HDFC Capital are investors (grant agreement vs. equity shares vs. convertible note) — Tracxn lists them as investors but the MCA cap table does not reflect equity stakes. Confirm MCA annual filing currency through FY24-25.',
        'vd-reg-dd': 'not-started',
  ...
  // and EIL fit over pure tech strength.
  ...
  function dwcsAdjusted() {
    ...
    return { profile: p, raw: totalScore(), adjusted: scaled,
      rows: [
        ...
        { l: 'EIL Fit', raw: e, max: 10, mult: p.eil }
      ]};
  }
  ...
  // Engagement gate — EIL strategic fit is now a hard minimum for any engagement.
  ...
  function generateReport() {
    ...
    const modules = [
      ...
      { l: 'EIL strategic fit', v: e, m: 10, c: '#0c447c', k: 'conf-m7' }
    ];
    ...
    radarChart = new Chart($('report-radar'), {
      type: 'radar',
      data: {
        labels: ['Technology', 'IP', 'Team', 'Market', 'Adoption', 'Business', 'EIL Fit'],
        datasets: [{
          label: 'Score %',
          data: [
    ...
    const valRows = [
      ...
      { k: 'strategic', l: 'Strategic Value (EIL)', v: strat, n: 'DCF + EIL/IP/team uplift' },
    ...
    // engagement cards
    const engOpts = [
      { k: 'equity', t: 'Equity investment', cond: 'CRS ≥ 85 · EIL Fit ≥ 7 · EPC ≥ 3', rec: 'Milestone-based tranches; IP co-dev clause; board observer; ROFR on EPCM.', col: '#0c447c', bg: '#e6f1fb' },
      { k: 'license', t: 'Licensing agreement', cond: 'CRS ≥ 70 · IP ≥ 10 · EIL Fit ≥ 5 · founder willing', rec: 'Exclusive field-of-use license; royalty escalators; independent IP valuation.', col: '#633806', bg: '#faeeda' },
      { k: 'engineering', t: 'Engineering partnership', cond: 'CRS ≥ 55 · EPC ≥ 3 · EIL Fit ≥ 4', rec: 'MSA with preferred-technology status; integrate in 1–2 upcoming projects.', col: '#155724', bg: '#d4edda' },
      { k: 'reject', t: 'Reject / watch list', cond: 'CRS < 50 · Tech < 10 · or EIL Fit < 3', rec: 'Structured gap analysis; 6-month watch; re-evaluate.', col: '#7b1d1d', bg: '#f8d7da' },
    ...
    if (e >= 7) recs.push({ cls: 'risk-info', txt: 'High EIL fit (' + e + '/10). Anchor EPCM partnership recommended.' });
    ...
    if (decision === 'reject') recs.push({ cls: 'risk-critical', txt: 'Recommended: Reject. CRS ' + tot + '/120 · Tech ' + t + '/25 · EIL Fit ' + e + '/10 — below minimum gate.' });
    ...
    // footer
    $('report-footer').innerHTML = `EIL EngSUI Platform · Evaluator: ${esc(gs('p-eval', 'EIL Strategy'))} · ${new Date().toLocaleDateString('en-IN', { day: 'numeric', month: 'long', year: 'numeric' })} · Confidential — internal use only`;
  }
  ...
  function renderPortfolioTable() {
    ...
    holder.innerHTML = `<div style="font-size:11px;color:var(--text-tertiary);margin-bottom:8px">
      Priority = CRS×40% + EIL×20% + Tech×15% + Market×15% + IP×10%. Ranked descending.
    </div>
  ...
  function validateImportSchema(obj) {
    ...
    if (obj.tool && obj.tool !== 'engsui-codex') return { ok: false, error: 'This file is not an EngSUI export.' };
